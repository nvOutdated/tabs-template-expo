...
export interface AreaTreeNode {
  ...
  export function getCascaderTree(data: CityNode[] = [], list: CascaderNode[] = []): CascaderNode[] {
     for (let i = 0; i < data.length; i++) {
        ...
        if (data[i].children) {
            ...
            if (data[i].name !== '市辖区') {
                list.push(son)
                getCascaderTree(data[i].children!, son.children)
            } else {
                getCascaderTree(data[i].children!, list)
            }
        } else {
            list.push(son)
        }  
  ...
  export function getCascaderTreeDisabledProvince(data: CityNode[] = [], list: CascaderNode[] = []): CascaderNode[] {
    for (let i = 0; i < data.length; i++) {
       ...
       if (data[i].children) {
           ...
           if (data[i].name !== '市辖区') {
               list.push(son)
               getCascaderTreeDisabledProvince(data[i].children!, son.children)
           } else {
               getCascaderTreeDisabledProvince(data[i].children!, list)
           }
       } else {
           list.push(son)
       }  
 ...
 export function getCascaderTreeDisabledCity(data: CityNode[] = [], list: CascaderNode[] = []): CascaderNode[] {
    for (let i = 0; i < data.length; i++) {
       ...
       if (data[i].children) {
           ...
           if (data[i].name !== '市辖区') {
               list.push(son)
               getCascaderTreeDisabledCity(data[i].children!, son.children)
           } else {
            getCascaderTreeDisabledCity(data[i].children!, list)
           }
       } else {
           list.push(son)
       }  
 ...
 export function foundAreaTree(list: AreaTreeNode[] = [], pid: string = 'pid', id: string = 'id'): AreaTreeNode[] {
  ...
  setChild(tree);
  // console.log(tree,list,99999);
  
  function setChild(tree: AreaTreeNode[]): void {
      if (list.length > 0) {
          for (let i = 0; i < tree.length; i++) {
              for (let j = 0; j < list.length; j++) {
                  if (list[j][pid] == tree[i][id]) {
                      let item = list[j];
                      item.label = item.name
                      item.key = item.area_id
              ...
              if (tree[i].children && tree[i].children![0]) {
                 ...
                 Reflect.deleteProperty(tree[i], 'children')
              }
          }
      }
  }
