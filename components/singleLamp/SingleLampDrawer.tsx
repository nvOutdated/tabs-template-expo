...
const DEVICE_STATUS = {
  OFFLINE: {
    condition: (info: Device["device_info"]) =>
      !info.online && !info.open && !info.warn,
    label: "离线",
    dotStyle: "offline",
...
const DeviceItem = ({
  ...
  const deviceStatus =
    Object.values(DEVICE_STATUS).find((status) =>
      status.condition(device.device_info)
...
export default function SingleLampDrawer({
  visible,
  onClose,
  areas,
  selectedDevice,
  onSelectDevice,
}: SingleLampDrawerProps){
  const currentTheme = useCurrentTheme();
    ...
    if (item.type === 'area') {
      ...
      return (
        <DeviceItem
          device={item.data}
          level={item.level}
          isSelected={selectedDevice?.id === item.data.id}
